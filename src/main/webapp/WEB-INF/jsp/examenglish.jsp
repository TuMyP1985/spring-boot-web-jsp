...
<div class="container">
    <form method="post" action="${pageContext.request.contextPath}/examenglish">
        <jsp:include page="fragments/examBodyHeader.jsp"/>
        <i style="color:#336b2f">Нужно выбрать правильное слово на англ.языке</i>
                ...
                <tr data-exam-error="${exam.error}">
                    ...
                    <td>
                        <select name="${status.index}" required>
                            <option value=${exam.val1} ${exam.result.equals(exam.val1) ? 'selected' : ''}>${exam.val1}</option>
                            <option value=${exam.val2} ${exam.result.equals(exam.val2) ? 'selected' : ''}>${exam.val2}</option>
                            <option value=${exam.val3} ${exam.result.equals(exam.val3) ? 'selected' : ''}>${exam.val3}</option>
                        </select>
                    </td>
